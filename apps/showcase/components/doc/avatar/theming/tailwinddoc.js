    ...
    avatar: {
        root: ({ props, state }) => ({
            className: classNames(
                ...
                {
                    'text-base h-8 w-8/12': props.size == null || props.size == 'normal',
                    'w-full h-12 text-xl': props.size == 'large',
                    'w-16 h-16 text-2xl': props.size == 'xlarge'
                },
                {
                    '-ml-6 border-2 border-white dark:border-gray-900': state.isNestedInAvatarGroup
                }
            )
        }),
        image: 'h-full w-full'
