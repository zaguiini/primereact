...
export function DataTableDoc(props) {
    ...
    const selectedProductContent = selectedProduct && (
        <div className="p-8 bg-surface-0 dark:bg-surface-900 shadow rounded-border">
            <div className="relative">
                <img src={'https://primefaces.org/cdn/primereact/images/product/' + selectedProduct.image} alt={selectedProduct.name} />
            </div>
            <div className="flex items-center justify-between mt-4 mb-2">
                <span className="text-surface-900 dark:text-surface-0 font-medium text-xl">{selectedProduct.name}</span>
                <span className="text-surface-900 dark:text-surface-0 text-xl ml-4">{'$' + selectedProduct.price}</span>
            </div>
            <span className="text-surface-600 dark:text-surface-200">{selectedProduct.category}</span>
        </div>
    );
    ...
    const imageBody = (rowData) => {
        return <img src={`https://primefaces.org/cdn/primereact/images/product/${rowData.image}`} alt={rowData.image} className="w-16 shadow-sm" />;
    };
