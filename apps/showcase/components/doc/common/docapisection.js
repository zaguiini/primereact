    ...
    const docs = doc.reduce((cDocs, name) => {
        ...
        if (mod) {
            const addToChildDoc = (childDoc, componentName) => {
                if (isNotEmpty(mod.events) && isNotEmpty(mod.events.values) && !isExcludedAll('events')) {
                    const eMap = {
                ...
                if (isNotEmpty(mod.interfaces) && isNotEmpty(mod.interfaces.values) && !isExcludedAll('interfaces')) {
                    const iMap = {
                ...
                if (isNotEmpty(mod.types) && isNotEmpty(mod.types.values) && !isExcludedAll('types')) {
                    const tMap = {
            ...
            if (splitedName.length === 3) {
                ...
                if (type === 'functions') {
                    ...
                    if (isNotEmpty(mod.interfaces) && isNotEmpty(mod.interfaces.values)) {
                        const iMap = {
                        ...
                        Object.entries(mod.interfaces.values).forEach(([iKey, iValue]) => {
                            if (types.includes(iKey)) {
                                ...
                                if (isNotEmpty(iValue.callbacks)) {
                                    tMap.children.push({
                                        id: `${id}.callbacks`,
                                        label: 'Callbacks',
                        ...
                        isNotEmpty(iMap.children) && fMap.children.push(iMap);
                    }
                ...
                mod.components &&
                    Object.entries(mod.components).forEach(([cKey, cValue]) => {
                        ...
                        if (isNotEmpty(cValue.props) && isNotEmpty(cValue.props.values) && !isExcludedAll('props')) {
                        ...
                        if (isNotEmpty(cValue.callbacks) && isNotEmpty(cValue.callbacks.values) && !isExcludedAll('callbacks')) {
                        ...
                        if (isNotEmpty(cValue.methods) && isNotEmpty(cValue.methods.values) && !isExcludedAll('methods')) {
                ...
                mod.model &&
                    Object.entries(mod.model).forEach(([mKey, mValue]) => {
                        ...
                        if (isNotEmpty(mValue.props) && isNotEmpty(mValue.props.values) && !isExcludedAll('props')) {
