...
export function ControlledDoc(props) {
    ...
    const headerTemplate = (
        <div className="p-4 pb-0">
            <Button type="button" icon="pi pi-plus" onClick={expandAll} className="w-8 h-8 mr-2 p-button-outlined" />
            <Button type="button" icon="pi pi-minus" onClick={collapseAll} className="w-8 h-8 p-button-outlined" />
        </div>
    );
...
<TreeSelect value={selectedNodeKey} onChange={(e) => setSelectedNodeKey(e.value)} options={nodes}
    className="md:w-80 w-full" placeholder="Select Item"
