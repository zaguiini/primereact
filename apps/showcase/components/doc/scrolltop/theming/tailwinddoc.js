    ...
    scrolltop: {
        root: ({ props }) => ({
            className: classNames('fixed bottom-20 right-20 flex items-center justify-center', 'ml-auto', {
                '!bg-blue-500 hover:bg-blue-600 text-white rounded-md h-8 w-8/12': props.target == 'parent',
                '!bg-gray-700 hover:bg-gray-800 h-12 w-full rounded-full text-white': props.target !== 'parent'
            })
        }),
        transition: {
    ...
    return (
        <div className="card">
            <div style={{ width: '250px', height: '200px', 'overflow': 'auto' }}>
                ...
                <ScrollTop target="parent" threshold={100} className="w-8 h-8 rounded-border bg-primary text-primary-contrast" icon="pi pi-arrow-up text-base" />
            </div>
        </div>
    );
}
