...
<Timeline value={events} opposite={(item) => item.status} 
    content={(item) => <small className="text-muted-color">{item.date}</small>} />
        `,
        javascript: `
...
export default function OppositeDemo() {
    ...
    return (
        <div className="card">
            <Timeline value={events} opposite={(item) => item.status} content={(item) => <small className="text-muted-color">{item.date}</small>} />
        </div>
    )
}
        `,
