    ...
    timeline: {
        ...
        opposite: ({ props, context }) => ({
            className: classNames(
                'flex-1',
                {
                    'px-6': props.layout === 'vertical',
                    'py-6': props.layout === 'horizontal'
                },
                {
        ...
        marker: 'flex self-baseline w-4/12 h-4 rounded-full border-2 border-blue-500 bg-white dark:border-blue-300 dark:bg-blue-900/40',
        connector: ({ props }) => ({
...
export default function UnstyledDemo() {
    ...
    return (
        <div className="card flex flex-wrap gap-12">
            <Timeline value={events} content={(item) => item.status} className="w-full md:w-80" />
            <Timeline value={events} align="right" content={(item) => item.status} className="w-full md:w-80" />
            <Timeline value={events} align="alternate" content={(item) => item.status} className="w-full md:w-80" />
        </div>
    )
}
    `
