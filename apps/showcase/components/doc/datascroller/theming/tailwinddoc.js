    ...
    const itemTemplate = (data) => {
        return (
            <div className="flex flex-col xl:flex-row xl:items-start p-6 gap-6">
                <img className="w-3/4 sm:w-64 xl:w-40 shadow block xl:block mx-auto rounded" src={\`https://primefaces.org/cdn/primereact/images/product/\${data.image}\`} alt={data.name} />
                <div className="flex flex-col lg:flex-row justify-between items-center xl:items-start lg:flex-1 gap-6">
                    <div className="flex flex-col items-center lg:items-start gap-4">
                        <div className="flex flex-col  gap-1">
                            <div className="text-2xl font-bold text-surface-900 dark:text-surface-0 dark:text-white/80">{data.name}</div>
                            <div className="text-surface-700 dark:text-surface-100 dark:text-white/80">{data.description}</div>
                        </div>
