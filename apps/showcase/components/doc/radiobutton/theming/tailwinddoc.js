    ...
    radiobutton: {
        ...
        input: {
              className: classNames(
                    'absolute appearance-none top-0 left-0 size-full p-0 m-0 opacity-0 z-10 outline-0 cursor-pointer'
              ),
        },
        box: ({ props }) => ({
            className: classNames(
                'flex justify-center items-center',
                'border-2 w-6/12 h-6 text-gray-700 rounded-full transition duration-200 ease-in-out',
                {
        ...
        icon: ({ props }) => ({
            className: classNames('transform rounded-full', 'block w-3/12 h-3 transition duration-200 bg-white dark:bg-gray-900', {
