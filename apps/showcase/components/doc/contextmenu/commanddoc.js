...
<ul className="m-0 p-0 list-none border border-surface rounded-border p-4 flex flex-col gap-2 w-full md:w-[30rem]">
    {users.map((user) => (
        <li
            key={user.id}
            className={\`p-2 hover:bg-emphasis rounded-border border border-transparent transition-all duration-200 flex items-center justify-between \${selectedUser?.id === user.id && 'border-primary'}\`}
