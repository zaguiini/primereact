...
export function TemplateDoc(props) {
    ...
    const nodeTemplate = (node, options) => {
        ...
        if (node.url) {
            label = (
                <a href={node.url} className="text-surface-700 dark:text-surface-100 hover:text-primary" target="_blank" rel="noopener noreferrer">
                    {node.label}
                </a>
            );
        }
...
export default function TemplateDemo() {
    ...
    const nodeTemplate = (node, options) => {
        ...
        if (node.url) {
            label = <a href={node.url} className="text-surface-700 dark:text-surface-100 hover:text-primary" target="_blank" rel="noopener noreferrer">{node.label}</a>;
        }
