...
const Tailwind = {
    datatable: {
        ...
        header: ({ props }) => ({
            className: classNames(
                'bg-slate-50 text-slate-700 border-gray-300 font-bold p-6',
        ...
        footer: {
            className: classNames(
                'bg-slate-50 text-slate-700 border-t-0 border-b border-x-0 border-gray-300 font-bold p-6',
                'dark:border-blue-900/40 dark:text-white/80 dark:bg-gray-900' // Dark Mode
            )
        },
        column: {
            headercell: ({ context, props }) => ({
                className: classNames(
                    'text-left border-surface-0 dark:border-surface-900 border-b border-solid border-gray-300 dark:border-blue-900/40 font-bold',
                    'transition duration-200',
                    context?.size === 'small' ? 'p-2' : context?.size === 'large' ? 'p-8' : 'p-6', // Size
            ...
            bodycell: ({ props, context }) => ({
                className: classNames(
                    'text-left border-surface-0 dark:border-surface-900 border-b border-solid border-gray-300',
                    context?.size === 'small' ? 'p-2' : context?.size === 'large' ? 'p-8' : 'p-6', // Size
            ...
            footercell: ({ context }) => ({
                className: classNames(
                    'text-left border-surface-0 dark:border-surface-900 border-b border-solid border-gray-300 font-bold',
            ...
            filteroverlay: {
                className: classNames(
                    'bg-white text-gray-600 border-surface-0 dark:border-surface-900 rounded-md min-w-[12.5rem]',
            ...
            filterrowitems: 'm-0 p-0 py-4 list-none ',
            filterrowitem: ({ context }) => ({
                className: classNames(
                    'm-0 py-4 px-8 bg-transparent',
                    'transition duration-200',
            ...
            filteroperator: {
                className: classNames(
                    'px-8 py-4 border-b border-solid border-gray-300 text-slate-700 bg-slate-50 rounded-t-md',
                    'dark:border-blue-900/40 dark:text-white/80 dark:bg-gray-900' // Dark Mode
                )
            },
            ...
            filterconstraint: 'p-8 border-b border-solid border-gray-300 dark:border-blue-900/40',
            filteraddrule: 'py-4 px-8',
            filteraddrulebutton: {
            ...
            filterbuttonbar: 'flex items-center justify-between p-8',
            filterclearbutton: {
                root: 'w-auto min-[0px]:text-sm border-blue-500 text-blue-500 px-6 py-4'
            },
            filterapplybutton: {
                root: 'w-auto min-[0px]:text-sm px-6 py-4'
            },
            filtermenubutton: ({ context }) => ({
                className: classNames(
                    'inline-flex justify-center items-center cursor-pointer no-underline overflow-hidden relative ml-2',
                    'w-8/12 h-8 rounded-[50%]',
                    'transition duration-200',
            ...
            headerfilterclearbutton: ({ context }) => ({
                className: classNames('inline-flex justify-center items-center cursor-pointer no-underline overflow-hidden relative', 'text-left bg-transparent m-0 p-0 border-0 select-none ml-2', {
                    invisible: !context.hidden
                })
            }),
            columnresizer: 'block absolute top-0 right-0 m-0 w-2/12 h-full p-0 cursor-col-resize border border-transparent',
            rowreordericon: 'cursor-move',
            roweditorinitbutton: {
                className: classNames(
                    'inline-flex items-center justify-center overflow-hidden relative',
                    'text-left cursor-pointer select-none',
                    'w-8/12 h-8 border-surface-0 dark:border-surface-900 rounded-[50%]',
                    'transition duration-200',
            ...
            radioButton: {
                className: classNames('relative inline-flex cursor-pointer select-none align-bottom', 'w-6/12 h-6')
            },
            radioButtonInput: {
                className: classNames(
                    'w-full h-full top-0 left-0 absolute appearance-none select-none',
                    'p-0 m-0 opacity-0 z-[1] rounded-[50%] outline-0',
                    'cursor-pointer peer'
                )
            },
            radioButtonBox: ({ context }) => ({
                className: classNames(
                    ...
                    'h-6 w-6/12 rounded-full border-2 text-gray-700 transition duration-200 ease-in-out',
                    context.checked
            ...
            radioButtonIcon: ({ context }) => ({
                className: classNames(
                    ...
                    'block h-3 w-3/12 bg-white transition duration-200 dark:bg-gray-900',
                    {
            ...
            headercheckboxwrapper: {
                className: classNames('cursor-pointer inline-flex relative select-none align-bottom', 'w-6/12 h-6')
            },
            headercheckbox: ({ context }) => ({
                className: classNames(
                    'flex items-center justify-center',
                    'border-2 w-6/12 h-6 text-gray-600 rounded-lg transition-colors duration-200',
            ...
            headercheckboxicon: 'w-4/12 h-4 transition-all duration-200 text-white text-base dark:text-gray-900',
            checkboxwrapper: {
                className: classNames('cursor-pointer inline-flex relative select-none align-bottom', 'w-6/12 h-6')
            },
            checkbox: ({ context }) => ({
            ...
            checkboxicon: 'w-4/12 h-4 transition-all duration-200 text-white text-base dark:text-gray-900',
        ...
        rowgrouptoggler: {
            className: classNames(
                ...
                'inline-flex items-center justify-center overflow-hidden relative',
                'w-8/12 h-8 text-gray-500 border-surface-0 dark:border-surface-900 bg-transparent rounded-[50%]',
                'transition duration-200',
                'dark:text-white/70' // Dark Mode
            )
        },
        rowgrouptogglericon: 'inline-block w-4/12 h-4',
...
export default function UnstyledDemo() {
    ...
    const filterFooterTemplate = () => {
        return <div className="px-4 pt-0 pb-4 text-center">Filter by Country</div>;
    };
    ...
    const activityFilterTemplate = (options) => {
        return (
            <React.Fragment>
                <Slider value={options.value} onChange={(e) => options.filterCallback(e.value)} range className="m-4"></Slider>
