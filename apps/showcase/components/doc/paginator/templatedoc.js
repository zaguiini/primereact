...
export function TemplateDoc(props) {
    ...
    const template1 = {
        ...
        PrevPageLink: (options) => {
            return (
                <button type="button" className={classNames(options.className, 'rounded-border')} onClick={options.onClick} disabled={options.disabled}>
                    <span className="p-4">Previous</span>
                    <Ripple />
                </button>
            );
        },
        ...
        CurrentPageReport: (options) => {
            return (
                <span className="mx-4" style={{ color: 'var(--text-color)', userSelect: 'none' }}>
        ...
        RowsPerPageDropdown: (options) => {
            return (
                <div className="flex items-center">
                    ...
                    <span className="mr-4" style={{ color: 'var(--text-color)', userSelect: 'none' }}>
                        Items per page:{' '}
                    </span>
...
<Paginator template={template2} first={first[1]} rows={rows[1]} totalRecords={120} onPageChange={(e) => onPageChange(e, 1)} className="justify-end" />
<Paginator template={template3} first={first[2]} rows={rows[2]} totalRecords={120} onPageChange={(e) => onPageChange(e, 2)} className="justify-start" />
        `,
        javascript: `
import React, { useState } from "react";
import { Button } from 'primereact/button';
