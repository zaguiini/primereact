...
export function TemplateDoc(props) {
    const itemRenderer = (item) => {
        if (item.root) {
            return (
                <a className="flex items-center cursor-pointer px-4 py-2 overflow-hidden relative font-semibold text-lg uppercase p-ripple" style={{ borderRadius: '2rem' }}>
            ...
            return (
                <a className="flex items-center p-4 cursor-pointer mb-2 gap-2 ">
                    <span className="inline-flex items-center justify-center rounded-full bg-primary text-primary-contrast w-12 h-12">
                        <i className={`${item.icon} text-lg`} />
                    </span>
                    <span className="inline-flex flex-col gap-1">
                        <span className="font-medium text-lg text-surface-900 dark:text-surface-0">{item.label}</span>
    ...
    const code = {
        basic: `
<MegaMenu model={items} orientation="horizontal" start={start} end={end} breakpoint="960px" className="p-4 bg-surface-0 dark:bg-surface-900 shadow" style={{ borderRadius: '3rem' }} />
`,
        javascript: `
...
export default function TemplateDemo() {
    const itemRenderer = (item, options) => {
        if (item.root) {
            return (
                <a className="flex items-center cursor-pointer px-4 py-2 overflow-hidden relative font-semibold text-lg uppercase p-ripple hover:bg-surface-50 dark:hover:bg-surface-950" style={{ borderRadius: '2rem' }} onClick={(e) => options.onClick(e)}>
                    <span className={item.icon} />
                    <span className="ml-2">{item.label}</span>
                    <Ripple />
                </a>
            ...
            return (
                <a className="flex items-center p-4 cursor-pointer mb-2 gap-2 " onClick={options.onClick}>
                    <span className="inline-flex items-center justify-center rounded-full bg-primary text-primary-contrast w-12 h-12">
            ...
            return (
                <div className="flex flex-col items-start gap-4" onClick={options.onClick}>
                    <img alt="megamenu-demo" src={item.image} className="w-full" />
                    <span>{item.subtext}</span>
                    <Button className="p-button p-component p-button-outlined" label={item.label} />
                </div>
