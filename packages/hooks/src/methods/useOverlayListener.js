import { getTargetElement, isTouchDevice } from '@primeuix/utils';
...
export const useOverlayListener = ({ target, overlay, listener, when = true, type = 'click' }) => {
    ...
    const [bindWindowResizeListener, unbindWindowResizeListener] = useResizeListener({
        target: 'window',
        listener: (event) => {
            listener && listener(event, { type: 'resize', valid: !isTouchDevice() });
        }
    });
    ...
    React.useEffect(() => {
        if (when) {
            ...
            overlayRef.current = getTargetElement(overlay);
        } else {
            unbind();
