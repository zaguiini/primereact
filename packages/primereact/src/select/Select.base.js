...
import { useEventListener, useId, useMountEffect, useOverlayListener, useUpdateEffect } from '@primereact/hooks';
...
export const useSelect = withComponent(({ props, attrs, parent, ref }) => {
    ...
    // element refs
    const elementRef = React.useRef(null); // @todo: remove
    ...
    const scrollInView = (index = -1) => {
        const _id = index !== -1 ? `${id}_${index}` : focusedOptionId;
        const element = findSingle(listRef.current, `li[id="${_id}"]`);
    ...
    const filterMessageText = React.useMemo(() => {
        return props.filterMessage || config.locale?.searchMessage || '';
    }, []);
    ...
    const emptyFilterMessageText = React.useMemo(() => {
        return props.emptyFilterMessage || config.locale?.emptySearchMessage || config.locale?.emptyFilterMessage || '';
    }, []);
    const emptyMessageText = React.useMemo(() => {
        return props.emptyMessage || config.locale?.emptyMessage || '';
    }, []);
    const selectionMessageText = React.useMemo(() => {
        return props.selectionMessage || config.locale?.selectionMessage || '';
    }, []);
    const emptySelectionMessageText = React.useMemo(() => {
        return props.emptySelectionMessage || config.locale?.emptySelectionMessage || '';
    }, []);
