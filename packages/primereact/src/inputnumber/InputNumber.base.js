...
import { focus, isAndroid, isEmpty, isLetter, isTouchDevice } from '@primeuix/utils';
import { defaultProps } from './InputNumber.props';

export const useInputNumber = withComponent(
    ({ props, parent, $primereact }) => {
        // states
        ...
        const _locale = props.locale || $primereact.config.locale;
        ...
        const spin = (event, dir) => {
            if (inputRef.current) {
                ...
                !isTouchDevice() && updateInput(newValue, null, 'spin');
        ...
        const allowMinusSign = () => {
            return isEmpty(props.min) || props.min < 0;
        };
        ...
        const insertText = (value, text, start, end) => {
            ...
            if (textSplit.length === 2) {
                ...
                const suffix = isLetter(value[end]) ? end - 1 : end;
        ...
        const maxBoundry = () => {
            return (props.value || props.defaultValue) >= props.max;
        };
        const minBoundry = () => {
            return (props.value || props.defaultValue) <= props.min;
        };
